 /**
 * SteeringPub.pp
 * Gigatron publisher for servo control using rosserial and Arduino to emulate steering
 * 
 * @author  Syler Wagner  <syler@mit.edu>
 * @date    2015-07-10    creation
 *
 ...
 * 2. Start SteeringPub in a separate window to control the servo.
 *  rosrun gigatron SteeringPub
 **/
...
int main(int argc, char **argv) {
   ...
   */
  ros::init(argc, argv, "SteeringPub");
   ...
   */
  ros::Publisher steering_pub = n.advertise<std_msgs::Int16>("steering", 1000);
   ...
   */
  int count = 0;
  ROS_INFO_STREAM("Increasing angle!");
  while (ros::ok() && count < 90)
  {
    /**
    ...
    ROS_INFO_STREAM("Steering angle: " << msg.data);
    /**
  ...
  ROS_INFO_STREAM("Decreasing angle!");
  while (ros::ok() && count > -90)   // count down
  {
    std_msgs::Int16 msg;
    msg.data = count;
  ...
  ROS_INFO_STREAM("I'll show you an angle...");
  while (ros::ok() && count < 0)   // count up again, but faster
  {
    std_msgs::Int16 msg;
    msg.data = count;
